...
func (h *Handler) login(c fiber.Ctx) error {
	...
	tokens, err := h.svc.IssueTokens(context.Background(), user, domain.Client{}, "gopenid", "openid profile email", "", true)
	if err != nil {
...
func (h *Handler) refresh(c fiber.Ctx) error {
	...
	user, stored, err := h.svc.ConsumeRefreshToken(context.Background(), req.RefreshToken)
	if err != nil {
	...
	tokens, err := h.svc.IssueTokens(context.Background(), user, client, stored.ClientID, stored.Scope, "", true)
	if err != nil {
		return httpx.Error(c, fiber.StatusInternalServerError, "token failed")
	}
...
func (h *Handler) logout(c fiber.Ctx) error {
	...
	if err := h.svc.RevokeAccessClaims(context.Background(), claims, "logout"); err != nil {
